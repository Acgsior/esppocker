import React from 'react';
import { useGrooming } from '../context/GroomingContext';
import { Check, Loader2, Eye, Users } from 'lucide-react';

export default function ParticipantList() {
    const { participants, currentUser, currentGrooming, actionBubble, hoveredVote } = useGrooming();

    const isRevealed = currentGrooming?.status === 'revealed';

    // Split voters and observers
    const voters = participants.filter(p => !p.is_observer);
    const observers = participants.filter(p => p.is_observer);

    // Calculate stats
    const votedParticipants = voters.filter(p => p.vote !== null);
    const totalParticipants = voters.length;
    const votedCount = votedParticipants.length;

    // Determine the highest vote(s)
    let highestVotes = new Set();
    if (isRevealed && votedCount > 0) {
        const counts = {};
        votedParticipants.forEach(p => {
            counts[p.vote] = (counts[p.vote] || 0) + 1;
        });
        const maxCount = Math.max(...Object.values(counts));
        highestVotes = new Set(Object.keys(counts).filter(k => counts[k] === maxCount));
    }

    return (
        <div className="bg-surface-card rounded-2xl shadow-sm border border-hairline p-6 max-lg:p-4">
            <div className="mb-6 max-lg:mb-3">
                <h2 className="text-xl font-bold text-ink">The Table</h2>
                <div className="flex justify-between items-center mt-2">
                    <h3 className="text-sm font-bold text-muted-soft uppercase tracking-wider flex items-center">
                        <Users className="w-4 h-4 mr-2" />
                        Voters
                    </h3>
                    <span className="text-sm font-medium bg-surface-soft text-body-strong py-1 px-3 rounded-full">
                        {votedCount} / {totalParticipants} Voted
                    </span>
                </div>
            </div>

            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-lg:gap-1.5">
                {voters.length === 0 ? (
                    <div className="col-span-full py-8 text-center text-muted text-sm">
                        Waiting for players to join...
                    </div>
                ) : (
                    voters.map((participant) => {
                        const hasVoted = participant.vote !== null;
                        const isHighest = isRevealed && hasVoted && highestVotes.has(participant.vote);
                        const isHovered = isRevealed && hoveredVote !== null && hoveredVote === participant.vote;
                        const isCurrentUser = currentUser?.id === participant.id;

                        return (
                            <div
                                key={participant.id}
                                className={`relative flex flex-col items-center p-4 max-lg:p-2 rounded-xl border-2 transition-all duration-500 ${isHovered
                                    ? 'border-coffee-300 bg-coffee-100 dark:bg-coffee-900/50 dark:border-coffee-700 shadow-md shadow-coffee-200/50 dark:shadow-none -translate-y-1 scale-105 z-10'
                                    : isHighest
                                        ? 'border-coffee-300 bg-coffee-50 dark:bg-coffee-900/40 dark:border-coffee-700'
                                        : hasVoted
                                            ? 'border-coffee-100 bg-coffee-50/30 dark:bg-coffee-900/20 dark:border-coffee-800'
                                            : 'border-dashed border-hairline bg-canvas/50'
                                    } ${isCurrentUser ? 'ring-2 ring-coffee-500 ring-offset-2 dark:ring-offset-surface-card' : ''}`}
                            >
                                {/* The "Card" on the table */}
                                <div
                                    className={`w-12 h-16 max-lg:w-10 max-lg:h-14 rounded-md shadow-sm mb-3 max-lg:mb-2 flex items-center justify-center overflow-hidden transition-all duration-500 ${isRevealed && hasVoted
                                        ? (isHighest ? 'bg-coffee-500 shadow-coffee-300 dark:shadow-none shadow-lg ring-2 ring-coffee-400 ring-offset-2 dark:ring-offset-surface-card [transform:rotateY(180deg)]' : 'bg-coffee-600 dark:bg-coffee-700 [transform:rotateY(180deg)] opacity-80')
                                        : hasVoted ? 'bg-coffee-200 dark:bg-coffee-800 border-2 border-coffee-300 dark:border-coffee-600' :
                                            'bg-surface-card border border-hairline'
                                        }`}
                                    style={{ perspective: '1000px', transformStyle: 'preserve-3d' }}
                                >
                                    {isRevealed && hasVoted ? (
                                        <div className={`font-bold select-none transition-all duration-500 tracking-tight flex items-center justify-center min-w-full min-h-full ${isHighest
                                                ? (participant.vote.length > 3 ? 'text-lg max-lg:text-sm text-white drop-shadow-md' : 'text-2xl max-lg:text-lg text-white drop-shadow-md')
                                                : (participant.vote.length > 3 ? 'text-base max-lg:text-xs text-stone-200' : 'text-lg max-lg:text-sm text-stone-200')
                                            }`} style={{ transform: 'rotateY(180deg)' }}>
                                            {typeof participant.vote === 'string' && participant.vote.includes('-') ? (
                                                <div className="flex flex-col items-center justify-center leading-none text-center text-s">
                                                    <span>{participant.vote.split('-')[0]}</span>
                                                    <span className="text-[0.7em] opacity-60 my-0.5">-</span>
                                                    <span>{participant.vote.split('-')[1]}</span>
                                                </div>
                                            ) : (
                                                participant.vote
                                            )}
                                        </div>
                                    ) : hasVoted ? (
                                        <Check className="w-5 h-5 max-lg:w-4 max-lg:h-4 text-coffee-500 dark:text-coffee-400" />
                                    ) : (
                                        <Loader2 className="w-4 h-4 max-lg:w-3 max-lg:h-3 text-muted-soft animate-spin" />
                                    )}
                                </div>

                                {/* Participant Name */}
                                <span className="text-sm max-lg:text-xs font-medium text-body-strong truncate w-full text-center">
                                    {participant.name} {isCurrentUser && <span className="text-coffee-600 dark:text-coffee-400 font-bold ml-1">(You)</span>}
                                </span>

                                {/* Status indicator pill */}
                                <span className={`text-[10px] max-lg:text-[9px] uppercase tracking-wider font-bold mt-1 max-lg:mt-0.5 px-2 max-lg:px-1.5 py-0.5 rounded-full transition-colors duration-500 ${isRevealed
                                    ? (isHighest ? 'text-orange-800 bg-orange-200 dark:bg-orange-900/50 dark:text-orange-300 shadow-sm' : (hasVoted ? 'text-muted bg-surface-soft' : 'text-muted-soft bg-surface-soft opacity-50'))
                                    : (hasVoted ? 'text-coffee-600 bg-coffee-100 dark:bg-coffee-900/50 dark:text-coffee-300' : 'text-muted bg-surface-soft animate-pulse')
                                    }`}>
                                    {isRevealed
                                        ? (isHighest ? 'Top Pick' : (hasVoted ? 'Voted' : 'Skipped'))
                                        : (hasVoted ? 'Ready' : 'Thinking')
                                    }
                                </span>

                                {/* Action Bubble */}
                                {actionBubble?.userId === participant.id && (
                                    <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-10 animate-float-up pointer-events-none drop-shadow-md">
                                        <div className={`text-white text-xs font-bold px-3 py-1.5 rounded-2xl shadow-lg relative flex items-center gap-1.5 whitespace-nowrap ${actionBubble.type === 'start' ? 'bg-coffee-800' : 'bg-orange-500'}`}>
                                            {actionBubble.type === 'start' ? (
                                                <><span className="text-sm">🔄</span><span>Start New!</span></>
                                            ) : (
                                                <><span className="text-sm">🤘</span><span>Open!</span></>
                                            )}
                                            <div className={`absolute w-2 h-2 transform rotate-45 -bottom-1 left-1/2 -translate-x-1/2 rounded-sm clip-bottom ${actionBubble.type === 'start' ? 'bg-coffee-800' : 'bg-orange-500'}`}></div>
                                        </div>
                                    </div>
                                )}
                            </div>
                        );
                    })
                )}
            </div>

            {observers.length > 0 && (
                <div className="mt-8 pt-6 max-lg:mt-4 max-lg:pt-4 border-t border-hairline">
                    <h3 className="text-sm font-bold text-muted-soft uppercase tracking-wider mb-4 max-lg:mb-2 flex items-center">
                        <Eye className="w-4 h-4 mr-2" />
                        Observers
                    </h3>
                    <div className="flex flex-wrap gap-3">
                        {observers.map(observer => {
                            const isCurrentUser = currentUser?.id === observer.id;
                            return (
                                <div key={observer.id} className={`relative flex items-center bg-canvas border border-hairline rounded-lg px-3 py-2 ${isCurrentUser ? 'ring-2 ring-coffee-500 dark:ring-offset-surface-card' : ''}`}>
                                    <div className="w-6 h-6 rounded-full bg-surface-soft flex items-center justify-center mr-2">
                                        <span className="text-xs font-bold text-body-strong">{observer.name.charAt(0).toUpperCase()}</span>
                                    </div>
                                    <span className="text-sm font-medium text-body-strong">
                                        {observer.name} {isCurrentUser && <span className="text-coffee-600 dark:text-coffee-400 font-bold ml-1">(You)</span>}
                                    </span>

                                    {/* Action Bubble for Observers */}
                                    {actionBubble?.userId === observer.id && (
                                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-10 animate-float-up pointer-events-none drop-shadow-md">
                                            <div className={`text-white text-xs font-bold px-3 py-1.5 rounded-2xl shadow-lg relative flex items-center gap-1.5 whitespace-nowrap ${actionBubble.type === 'start' ? 'bg-coffee-800' : 'bg-orange-500'}`}>
                                                {actionBubble.type === 'start' ? (
                                                    <><span className="text-sm">🔄</span><span>Start New!</span></>
                                                ) : (
                                                    <><span className="text-sm">🤘</span><span>Open!</span></>
                                                )}
                                                <div className={`absolute w-2 h-2 transform rotate-45 -bottom-1 left-1/2 -translate-x-1/2 rounded-sm clip-bottom ${actionBubble.type === 'start' ? 'bg-coffee-800' : 'bg-orange-500'}`}></div>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            );
                        })}
                    </div>
                </div>
            )}
        </div>
    );
}
